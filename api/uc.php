...
define('DISCUZ_ROOT', substr(dirname(__FILE__), 0, -3)); //定义DISCUZ_ROOT为api上层的目录，用于获取配置文件

require_once  DISCUZ_ROOT.'/ucenter.inc'; // ucenter_call.inc中的函数用到里面的函数
require_once  DISCUZ_ROOT.'/ucenter_call.inc';
require_once  DISCUZ_ROOT.'/config.inc.php'; //把密钥加载进来，用于解密请求。
...
if(in_array($get['action'], array('test', 'deleteuser', 'renameuser', 'gettag', 'synlogin', 'synlogout', 'updatepw', 'updatebadwords', 'updatehosts', 'updateapps', 'updateclient', 'updatecredit', 'getcreditsettings', 'updatecreditsettings'))) {
    /*
    require_once DISCUZ_ROOT.'./uc_client/include/db_'.UC_CONNECT.'.class.php';
    $GLOBALS['db'] = new dbstuff;
    $GLOBALS['db']->connect(UC_DBHOST, UC_DBUSER, UC_DBPW, UC_DBNAME, UC_DBCONNECT, true, UC_CHARSET);
...
class uc_note {
    ...
    function renameuser($get, $post) {
        ...
        if (ucenter_renameuser($uid,$usernameold,$usernamenew)){
            return API_RETURN_SUCCEED;
        }
        else{
            return API_RETURN_FAILED;
    ...
    function synlogin($get, $post) {
        ...
        if (ucenter_synlogin($uid,$username)){
            return API_RETURN_SUCCEED;
        }
        else{
            return API_RETURN_FAILED;
    ...
    function updatepw($get, $post) {
        ...
        require_once './includes/bootstrap.inc';    
        drupal_bootstrap(DRUPAL_BOOTSTRAP_FULL);
        if(ucenter_updatepw($username,$password)){
            return API_RETURN_SUCCEED;
        }
        else{
            return API_RETURN_FAILED;
    ...
    function updatebadwords($get, $post) {
        ...
        $cachefile = $this->appdir.'./data/cache/badwords.php';
    ...
    function updatehosts($get, $post) {
        ...
        $cachefile = $this->appdir.'./data/cache/hosts.php';
        $fp = fopen($cachefile, 'w');
        $s = "<?php\r\n";
    ...
    function updateapps($get, $post) {
        ...
        //note 写 app 缓存文件
        $cachefile = $this->appdir.'./data/cache/apps.php';
        $fp = fopen($cachefile, 'w');
        $s = "<?php\r\n";
    ...
    function updateclient($get, $post) {
        ...
        $cachefile = $this->appdir.'./data/cache/settings.php';
        $fp = fopen($cachefile, 'w');
        $s = "<?php\r\n";
